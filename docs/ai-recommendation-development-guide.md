...
2026-08-10 공식 모델 목록과 추천 서비스의 제한된 작업 범위를 검토해 기본값은
`gpt-5.6-luna`로 설정했습니다. 제공된 후보 중 최대 3개를 고르는 작업은 복잡한 코드 생성보다
비용 효율과 반복 호출 비용이 중요하므로 비용 민감 워크로드용 Luna를 선택했습니다. 모델은
`OPENAI_MODEL` 환경변수로 분리했으므로 품질 요구가 바뀌면 배포 설정만 교체할 수 있습니다.

...
- 플랫폼 운영 화면은 아직 목업이다. user-service 소유 사용자·기업 운영 API는 구현됐지만 화면 라이브 연동과 payment-service·enrollment-service 소유 결제·수강 운영 API는 남아 있다. 일부 API 구현 완료와 전체 운영 기능·화면 완료를 같은 의미로 공유하면 안 된다.

...
| `recommend-service` | 언어·수준·직무·상황·목표 기반 추천과 실제 `ACTIVE` 강의 검증 | 김지민 팀원 최종 연동 예정 |

현재 전체 흐름은 `기업 구독·결제 → Kafka 이벤트 → 기업 권한·좌석 → 직원 초대 → 강의 검색·수강신청 → 학습·진도 → 기업·플랫폼 운영`까지 동작합니다. OAuth2와 공개·보호·내부 API 79건 및 역할별 Chrome 상황을 2026-08-11 통합 검증했습니다. 추천 요청은 팀원 연동 전 API 골격 검증이므로 최종 추천 시스템은 병합 후 별도로 재검증합니다.

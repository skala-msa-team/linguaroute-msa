...
| 제공하지 않는 강의가 노출될 수 있음 | 강의 등록·수정·비활성화 기능 제공 |
...
| 강의 관리 | 플랫폼 관리자 | 강의 등록·수정·비활성화 |
| 강의 관리 | 플랫폼 관리자 | 강의 차시 등록 |
| 수강신청 | 직원 | 개별 강의 수강신청 |
| 수강신청 | 시스템 | 중복 수강신청 방지 |
| 수강신청 | 직원 | 신청한 강의 목록 조회 |
| 수강 관리 | 직원 | 강의 차시 목록 조회 및 선택한 차시 수강 |
| 수강 관리 | 직원 | 차시 완료 처리 |
...
| 인증 | 전체 사용자 | 이메일 인증 | 필수 |
| 인증 | 전체 사용자 | 아이디 찾기 | 선택 |
| 강의 관리 | 플랫폼 관리자 | 강의 게시·비게시 상태 관리 | 필수 |
...
직원 조건 입력
→ AI가 조건 분석
→ course-service에서 활성 강의 검색
→ 선택 언어와 일치하는 강의만 필터링
→ 추천 이유와 함께 결과 반환
```

AI는 존재하지 않는 강의를 새로 만들어 추천하지 않습니다. 반드시 `course-service`에 등록되어 있고 `ACTIVE` 상태이며, 직원이 선택한 언어와 일치하는 강의만 추천합니다. AI 호출이 실패하면 규칙 기반 대체 추천을 반환합니다.
...
| 박건우 | 구독·결제 | `payment-service`, Kafka | 요금제, 구독, 결제, 이벤트 |
| 김지민 | AI 추천·통합 | 추천 영역, 프론트 공통 | AI 추천, 공통 UI, Gateway 연동, 통합 테스트 |

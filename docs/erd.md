...
상태 변경과 이벤트 발행 사이의 유실을 방지하기 위해 Transactional Outbox 방식을 권장합니다.
...
| 제공 Auth Server 기능 | 목표 인증 API별 기존 지원 여부 확인, 미지원 시 수정 가능한 Auth Server 소스 확보 |
...
| 정기결제 범위 | 모의 결제와 `payment-service`의 기간·갱신·만료 관리로 구현 |
| 인증 이메일 | SMTP 링크, 토큰 유효시간 15분, 일회성 사용과 요청 횟수 제한 적용 |
| 이벤트 전달 보장 | Outbox 발행 재시도와 `processed_event` 보관 기간 확정 |

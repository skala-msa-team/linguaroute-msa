...
def test_gateway_blocks_internal_api_paths_before_public_routes():
    ...
    assert "SPRING_CLOUD_GATEWAY_ROUTES_0_FILTERS_0=SetStatus=404" in compose
    assert "SPRING_CLOUD_GATEWAY_ROUTES_0_ORDER=-100" in compose
    assert compose.index(block_id) < compose.index(
        "SPRING_CLOUD_GATEWAY_ROUTES_3_ID=course-service"
    )
...
def test_gateway_keeps_recommendation_public_route_separate():
    ...
    assert "SPRING_CLOUD_GATEWAY_ROUTES_2_ID=recommend-service" in compose
    assert (

...
LinguaRoute는 기업이 월간 또는 연간 구독권을 구매하고, 초대받은 직원이 업무에 바로 적용할 수 있는 비즈니스 외국어를 학습하는 B2B2E 외국어교육 서비스이다. 직원은 언어·수준·직무·비즈니스 상황과 제품 사양·기술 용어 같은 자연어 학습 목표를 입력해 AI 강의 추천도 받을 수 있다.
...
- `recommend-service`: 직무·비즈니스 상황·전문용어 목표 기반 추천, 추천 결과 검증과 저장
...
아래 화면은 LinguaRoute 랜딩 화면과 Swagger에서 확인한 수강 API 목록이다. 화면을 만드는 데서 끝내지 않고, 프론트엔드가 호출할 경로와 요청 대상을 Swagger에서 확인한 뒤 실제 연동에 사용했다.

![LinguaRoute 실제 랜딩 화면](./assets/linguaroute-landing.png)

그림 3. LinguaRoute 실제 랜딩 화면

![Swagger 수강 API 목록](./assets/swagger-enrollment.png)
...
잘한 점은 주어진 교육용 코드를 화면만 바꾼 데서 끝내지 않고 LinguaRoute의 B2B2E 흐름에 맞게 구독, 좌석, 초대, 학습과 추천을 실제 서비스 책임으로 연결한 것이다. 일반 회화 추천이 아니라 해외 고객 미팅·업무 발표 같은 실제 비즈니스 시나리오와 제품 사양·기술 용어 학습 목표를 연결하도록 기획한 점도 차별점으로 삼았다. 특히 내가 맡은 구독·결제에서는 중복 요청, 이벤트 발행 실패와 중복 소비를 고려해 멱등키, Outbox와 `eventId` 처리까지 구현했다. 프론트도 최종적으로 Gateway의 실제 API를 사용하도록 연결했다.
...
교수님께서 일반적으로 쉽게 따라 할 수 없는 비즈니스 시나리오, 도메인 이해와 기술 용어를 자신의 차별점으로 가져가야 한다고 말씀하신 부분도 기억에 남았다. 그래서 LinguaRoute를 단순한 외국어 학습이 아니라 해외 고객 미팅, 업무 발표와 제품 사양·기술 용어 학습을 연결하는 비즈니스 외국어 서비스로 설명하려고 했다. 처음에는 새로운 기술을 많이 사용하는 것이 곧 차별점이라고 생각했다. 이번 프로젝트를 마친 뒤에는 같은 MSA와 AI를 사용하더라도 어떤 사용자의 문제를 해결하는지, 왜 이 서비스 경계와 데이터 저장 방식을 선택했는지를 설명할 수 있어야 진짜 차별점이 된다고 생각하게 되었다.

...
const useLiveApi=import.meta.env.VITE_USE_LIVE_API==='true'; const verificationCode=ref(''); const passwordConfirm=ref(''); const formError=ref('')
...
async function requestCompanyVerification(){formError.value='';try{if(!companyForm.admin.email){formError.value='이메일을 입력해 주세요.';return}if(useLiveApi)await authApi.requestEmailVerification(companyForm.admin.email);verificationSent.value=true}catch(error){formError.value=error.response?.data?.message||'인증번호를 발송하지 못했습니다.'}}
async function confirmCompanyVerification(){formError.value='';try{if(!verificationCode.value){formError.value='인증번호를 입력해 주세요.';return}if(useLiveApi){const response=await authApi.confirmEmailVerification(companyForm.admin.email,verificationCode.value);companyForm.emailVerificationToken=response.data.data.emailVerificationToken}else companyForm.emailVerificationToken='prototype-email-verification-token'}catch(error){formError.value=error.response?.data?.message||'이메일 인증에 실패했습니다.'}}

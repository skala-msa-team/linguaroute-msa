<template>
  <AppShell>
    ...
    <PageHeader :eyebrow="isEdit ? 'Edit course' : 'Create course'" :title="isEdit ? '강의 수정' : '새 강의 등록'" description="최신 강의 enum과 차시 계약을 기준으로 콘텐츠를 구성하세요."><button class="button accent" @click="saveCourse">{{ saved ? '저장 완료' : '강의 저장' }} <CircleCheck v-if="saved" :size="16" /><Save v-else :size="16" /></button></PageHeader>
...
async function saveCourse() { if (useLiveApi) { if (isEdit.value) await courseApi.update(route.params.id, form); else await courseApi.create(form) } saved.value = true }
</script>

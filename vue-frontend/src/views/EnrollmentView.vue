...
const activeCourses = computed(() => liveCourses.value.length ? liveCourses.value : learningCourses.value)
...
onMounted(async () => {
  ...
  liveCourses.value = await Promise.all(response.data.data.map(async (enrollment) => {
    ...
    try {
      ...
      return { ...fallback, ...course, id: enrollment.courseId, enrollmentId: enrollment.enrollmentId,
        progress: Number(enrollment.progressRate), nextLessonId: lessons.find((lesson) => lesson.required)?.lessonId,
        duration: '등록된 차시 기준', students: '-', tone: 'green' }
    } catch (_) {
      return { ...fallback, id: enrollment.courseId, enrollmentId: enrollment.enrollmentId, progress: Number(enrollment.progressRate), nextLessonId: null }
    }
  }))
})
</script>
